...
class MyMainWindow(QMainWindow):
    ...
    def HelpWindow(self):
        ...
        self.bodies = [QLabel("Tube Sorter Worklist Generator can take an input CSV (comma-separated values) file for a single 96-tube matrix rack from a SampleScan Mini (SSMINI), and output a CSV worklist for the BioMicroLab XL20. The requirements are as follows:\n\n"
                              ...
                              "\t-The number of rows cannot exceed 96 (one full matrix rack). However, the number can be less than 96, and worklists for partial matrix racks can be generated. Although the SSMINI will always output 96 rows containing data for every single position on the matrix rack, the positions that have no tube present will be excluded when the worklist is generated.\n"
    ...
    def CreateWorklist(self):
        ...
        if destinationName == "":
            ...
            if fileName == "":
                ...
                if df.shape[0] > 96 or df.shape[1] > 4:
                    self.submit_result.setStyleSheet("color: red; font: bold 12pt Times MS")
